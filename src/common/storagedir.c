 ...
 * stored in this format can be recovered with storage_dir_map_labelled
 * or storage_dir_read_labelled().
 */
int
storage_dir_save_labelled_to_file(storage_dir_t *d,
...
/**
 * Map a file that was created with storage_dir_save_labelled().  On failure,
 ...
 */
tor_mmap_t *
storage_dir_map_labelled(storage_dir_t *dir,
                         const char *fname,
                         ...
                         size_t *sz_out)
{
  ...
  if (! nulp)
    goto err;
  if (config_get_lines(m->data, labels_out, 0) < 0)
    goto err;
...
/** As storage_dir_map_labelled, but return a new byte array containing the
 * data. */
uint8_t *
storage_dir_read_labelled(storage_dir_t *dir,
                          const char *fname,
                          config_line_t **labels_out,
                          size_t *sz_out)
{
  const uint8_t *data = NULL;
  tor_mmap_t *m = storage_dir_map_labelled(dir, fname, labels_out,
                                           &data, sz_out);
  if (m == NULL)
    return NULL;

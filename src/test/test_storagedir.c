...
test_storagedir_save_labeled(void *arg)
{
  ...
  int r = storage_dir_save_labelled_to_file(d, labels, inp, 8192, &fname);
  tt_int_op(r, OP_EQ, 0);
...
test_storagedir_read_labeled(void *arg)
{
  ...
  size_t sz = 0;
  map = storage_dir_map_labelled(d, fname, &labels, &datap, &sz);
  tt_assert(map);
  tt_assert(datap);
  ...
  sz = 0;
  as_read = storage_dir_read_labelled(d, fname, &labels2, &sz);
  tt_assert(as_read);
  tt_u64_op(sz, OP_EQ, bodylen);
